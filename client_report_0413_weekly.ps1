...
foreach ($entity in $objectStatus.Keys | Sort-Object){
    ...
    {$Cancelednum += 1} elseif ($status -eq 'Success'){$Successnum += 1} elseif($status -eq 'Warning'){$Warningnum += 1}
     ...
     $allcluster[$clusterName]['wtotalFailedObjects'] += $totalFailedObjects
         ...
         "{0,-10}  {1,-10}  {2,-10} {3,-10}  {4,-10} {5,-10}" -f "ClusterName","Total_Failed","Total_Running","Total_Canceled","Total_Success","Total_Warning"
         "=================================================================================="
         "{0,-10}     {1,-10}     {2,-10}    {3,-10}    {4,-10}   {5,-10}" -f $_.name,$_.value.wtotalFailedObjects,$_.value.wRunningnum,$_.value.wCancelednum,$_.value.wSuccessnum,$_.value.wWarningnum
