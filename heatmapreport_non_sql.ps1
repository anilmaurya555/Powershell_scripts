...
$title = "Heatmap Report(Report Card) for $($cluster.name)"
...
$html += $fileDate
$html += '</table>
<p style="margin-top: 15px; margin-bottom: 15px;"><span style="font-size:1em;color: #0000FF"> Below report also availbe on NAS share : \\hcohesity05\cohesity_reports </span></span></p>
</html>'
$html += "</span>
</p>
<table>
...
foreach($item in $report | Sort-Object -Property name){
    if ( $item.environment.subString(1) -notlike "SQL"){
