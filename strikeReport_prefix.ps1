    ...
    ######
                  ...
                  $includeJob = $false
                        foreach($pre in $prefix){
                                 ...
                                 if ($job.name.tolower().startswith($pre.tolower()) -or $prefix -eq 'ALL') {
                                                     $includeJob = $True
                                                       }
                                                }
                                        if($includeJob){
...
foreach($toaddr in $sendTo){
    #Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject "$prefixTitle Backup strike report from ($($cluster.name))" -BodyAsHtml $html -Attachments $fileName -WarningAction SilentlyContinue
    Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject "$prefixTitle Backup strike report from ($($cluster.name))" -BodyAsHtml $html -WarningAction SilentlyContinue
