...
#$clusters = ("Hcohesity03","Hcohesity05")
    ...
    foreach($runClass in $runClasses){
        ...
        while($runCount -gt 0){
            ...
            foreach($protectionRun in $theseRuns.backupJobRuns.protectionRuns){
                ...
                # "    {0}" -f (usecsToDate $runStartTimeUsecs)
                $tasks = @(($protectionRun.backupRun.latestFinishedTasks) + ($protectionRun.backupRun.activeAttempt.activeTasks))
