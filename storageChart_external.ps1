...
if($showCapacity){
	$stats2 = api get statistics/timeSeriesStats?schemaName=kBridgeClusterStats`&entityId=$($vault.id)`&metricName=kCapacityBytes`&startTimeMsecs=$startTimeMsecs`&rollupFunction=average`&rollupIntervalSecs=86400
}
...
$outFilePath = join-path -Path $PSScriptRoot -ChildPath "$clusterName-$($vault.name).html"
...
foreach($toaddr in $sendTo){
    Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject "$prefixTitle IBM COS Bucket consumption report from ($($clustername))" -BodyAsHtml $html2 -Attachments $outfilepath -WarningAction SilentlyContinue
}
