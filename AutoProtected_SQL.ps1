[CmdletBinding()]
param (
    #[Parameter(Mandatory = $True)][string]$vip, # Cohesity cluster to connect to
    [Parameter(Mandatory = $True)][string]$username #cohesity username
    #[Parameter()][string]$domain = 'local' # local or AD domain
    #[Parameter()][string]$smtpServer, # outbound smtp server '192.168.1.95'
    #[Parameter()][string]$smtpPort = 25, # outbound smtp port
    #[Parameter()][array]$sendTo, # send to address
    #[Parameter()][string]$sendFrom # send from address
)

# source the cohesity-api helper code
. $(Join-Path -Path $PSScriptRoot -ChildPath cohesity-api.ps1)

#################################################
######HTML coding ###########################
$title = "SQL Server Protection Summary Report as of "
$date = (get-date).ToString()
$html = '<html>
<head>
    <style>
        p {
            color: #555555;
            font-family:Arial, Helvetica, sans-serif;
        }
        span {
            color: #555555;
            font-family:Arial, Helvetica, sans-serif;
        }
        
        table {
            font-family: Arial, Helvetica, sans-serif;
            border: 3px solid maroon;
            font-size: 0.75em;
            border-collapse: collapse;
             width:25%;
		 margin:0 auto;
            
        }
        tr {
            border: 1px solid #F1F1F1;
        }
        td {
            border:2px solid blue;
		   padding:10px;
            text-align: left;
            #background-color: #EAFAF1;
            
        }
        th {
            border:2px solid blue;
		   padding:10px;
            text-align: left;
            font-size:1em;
            background-color: #FFC300;
            
        }
        tr:nth-child(even) {
            background-color: #FADBD8;
        }
    </style>
</head>
<body>
    
    <div style="margin:15px;">
        <img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAARgAAAAoCAMAAAASXRWnAAAC8VBMVE
        " style="width:180px">
        <p style="margin-top: 15px; margin-bottom: 15px;">
            <span style="font-size:1.3em;">'

$html += $title
$html += '</span>
<span style="font-size:1em; text-align: right; padding-top: 300px; padding-right: 100px; float: center;">'
$html += $date

$html += '</table>
<p style="margin-top: 15px; margin-bottom: 15px;"><span style="font-size:0.5em;color: #0000FF"> Below report also availbe on NAS share : \\hcohesity05\cohesity_reports </span></span></p>
</html>'

$html += '</span>
</p>
<table>
<tr>
    <th>Cluster Name</th>
    <th>SQL server</th>
    <th>Protection Status</th>
    
</tr>'
################################################

$report = @{}
$allid = @()
$groups = @()
$sourceids = @()
##############

#$clusters = ("Hcohesity01","Hcohesity03","Hcohesity04","Hcohesity05")
#$clusters = ("Hcohesity01","Hcohesity03")
$clusters = ("Hcohesity05")
foreach ($cluster in $clusters){
apiauth -vip $cluster -username $username -domain corpads.local
$jobs = api get -v2 data-protect/protection-groups?environments=kSQL
$bjob = api get protectionJobs | Where-Object {$_.isdeleted -ne $True -and $_.isactive -ne $false -and $_.name -like "*sql*"}
$groups = $bjob.name
$sources = api get protectionSources?environments=kSQL
$allid = $jobs.protectionGroups.mssqlParams.fileProtectionTypeParams.objects.id

        foreach ($group in $groups){
                 $njob = api get protectionJobs | Where-Object {$_.name -ieq $group}
                 $sourceids = $njob.sourceids
                        foreach ($source in $sources[0].nodes.protectionSource.name) {
                        $node = $sources.nodes | Where-Object { $_.protectionSource.name -eq $source  }
                                                       
                                          if ($source -notin $report.keys -and $node.protectionsource.id -in $sourceids){
                                                          $report[$source] =@{}
                                                          $report[$source]['cluster'] = $cluster
                                                          #$node.protectionsource.id
                                                          $report[$source]['id'] = $node.protectionsource.id
                                                          if ($node.protectionsource.id -in $allid){
                                                                          $report[$source]['protection'] = "AutoProtected"
                                                                          }
                                                                          elseif ( $group -notlike "*volume*" )
                                                                          {$report[$source]['protection'] = "Not_AutoProtected"}
                                                                          else  {                                                                       
                                                                          $report[$source]['protection'] = "Volume_Based"
                                                                           }
                                                                           
                                                           }

                                                          }
                                    }
                                    
                                   }

$report.GetEnumerator()|Sort-Object -Property {$_.Value.cluster} |ForEach-Object { 
######################################
#######Ending HTML code ##########
if ( $_.value.protection -like "*Not*") { $color='FF0000'} else { $color='00ff00'}
$HTML += "               <tr><td>{0}</td>
                              <td>{1}</td>
                               <td bgcolor= '#$($color)'>{2}</td></tr>" -f $_.value.cluster,$_.name,$_.value.protection

           }
                                 
#####################################

$html += '</table>
<p style="margin-top: 15px; margin-bottom: 15px;"><span style="font-size:0.5em;color: #0000FF">Contact: Anil Maurya for any Question/Comment about this report.</p>
</div>
</body>
</html>
'



$fileName = "./SQL_servers_Protection_summary_report.html"
$html | out-file $fileName

### send email report
#foreach($toaddr in $sendTo){
    #Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject "SQL Servers Prtection Summary Report" -BodyAsHtml $html -Attachments $fileName -WarningAction SilentlyContinue
#}
#copy report to NAS share
$today = get-date
$targetPath = '\\hcohesity05.corpads.local\cohesity_reports'
$year = $today.Year.ToString()
$month = $today.Month.ToString()
$date  =  $today.date.ToString('MM-dd') 
# Set Directory Path
$Directory = $targetPath + "\" + $year + "\" + $month + "\" + $date
# Create directory if it doesn't exsist
if (!(Test-Path $Directory))
{
New-Item $directory -type directory
}
# copy File to NAS location
$filename | Copy-Item -Destination $Directory
#######################################################
