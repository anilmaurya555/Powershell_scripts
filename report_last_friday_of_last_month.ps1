[CmdletBinding()]
param (
    
    [Parameter()][string]$smtpServer, #outbound smtp server '192.168.1.95'
    [Parameter()][string]$smtpPort = 25, #outbound smtp port
   [Parameter()][array]$sendTo, #send to address
   [Parameter()][string]$sendFrom #send from address
)
# process email body
 $title = "Thank you "
$html = '<html>
<body>
    <div style="margin:15px;">
            <img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAARgAAAAoCAMAAAASXRWnAAAC8VBMVE
            " style="width:180px">
        <p style="margin-top: 15px; margin-bottom: 15px;">
            <span style="font-size:1.3em;">'

$html += '</table>
<p style="margin-top: 15px; margin-bottom: 15px;"><span style="font-size:1em;color: #0000FF"> Attached report also availbe on NAS share : \\hcohesity05\cohesity_reports </span></span></p>
</html>'
$html += $title

$twoMonth = [datetime]::Now.AddMonths(-2).ToString('yyyy\\M')
$TargetFolder1 = Get-ChildItem (Join-Path '\\hcohesity05\cohesity_reports' $twoMonth) |Where{$_.CreationTime.DayOfWeek -eq 'Friday'}|Sort CreationTime|Select -Last 1
$FilePath1 = (Resolve-Path (Join-Path $TargetFolder1.FullName 'Cohesity_FETB_Report-*-14-26-48.xlsx')).Path
$oldbk = Import-Excel -Path $FilePath1
#$FilePath1

$LastMonth = [datetime]::Now.AddMonths(-1).ToString('yyyy\\M')
$TargetFolder2 = Get-ChildItem (Join-Path '\\hcohesity05\cohesity_reports' $LastMonth) |Where{$_.CreationTime.DayOfWeek -eq 'Friday'}|Sort CreationTime|Select -Last 1
$FilePath2 = (Resolve-Path (Join-Path $TargetFolder2.FullName 'Cohesity_FETB_Report-*-14-26-48.xlsx')).Path
#$newbk = Import-Excel -Path $FilePath2
#$FilePath2

Import-Excel -Path $FilePath2 | Where-Object {
    $_."Server Name" -notin $oldbk."Server Name"
} | Export-Excel -Path '.\New_capacity_added_lastMonth.xlsx'

$xlsx = Join-Path -Path (Get-Location).Path -ChildPath "New_capacity_added_lastMonth.xlsx"
# send email report
#write-host "sending report to $([string]::Join(", ", $sendTo))"
foreach($toaddr in $sendTo){
   Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject "New Capacity added Last Month to Cohesity Enviornment." -BodyAsHtml $html  -WarningAction SilentlyContinue -Attachments $xlsx }
#$html | out-file "$($cluster.name)-objectreport.html"
#copy report to NAS share
$today = get-date
$targetPath = '\\hcohesity05.corpads.local\cohesity_reports'
$year = $today.Year.ToString()
$month = $today.Month.ToString()
$date  =  $today.date.ToString('MM-dd') 
# Set Directory Path
$Directory = $targetPath + "\" + $year + "\" + $month + "\" + $date
# Create directory if it doesn't exsist
if (!(Test-Path $Directory))
{
New-Item $directory -type directory
}
# copy File to NAS location
$xlsx | Copy-Item -Destination $Directory