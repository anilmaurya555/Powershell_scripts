...
#$vips = ("chyusnpccp03","chyuswpccp03")
...
if($runningTasks.Keys.Count -gt 0){
    ...
    $clusterstats[$vip]['jobname']   = "No active replication tasks found"
}
...
foreach($toaddr in $sendTo){
   Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject "Oldest Running Replication stats from ALL US Cohesity cluster." -BodyAsHtml $html -WarningAction SilentlyContinue }
