...
$4days = $(timeAgo 15 days)/1000

if ( $VIP -in ('Hcohesity01','Hcohesity03')) {
                                                $TB = (1024*1024*1024*1024)
                                              if ($VIP -eq 'Hcohesity01'){
                                                        $vault = api get vaults | Where-Object name -eq 'HC1Archive1_ICOS'
                                                        #$stats = api get "statistics/timeSeriesStats?entityId=$($vault.id)&metricName=kMorphedUsageBytes&metricUnitType=0&range=day&schemaName=kIceboxVaultStats&startTimeMsecs=$todayMsecs”
                                                        $stats = api get "statistics/timeSeriesStats?entityId=$($vault.id)&metricName=kMorphedUsageBytes&metricUnitType=0&range=month&rollupFunction=latest&rollupIntervalSecs=86400&schemaName=kIceboxVaultStats&startTimeMsecs=$4days"
           ...
           $_.value.changerate.GetEnumerator()| Sort-Object { $_.name}| ForEach-Object  {
                             ...
                             #"{0,-20}  {1,10}" -f $_.name,$_.value
                                ...
                                '
                                                                        ...
                                                                        $html +='
                                                                        <tr> <td class="td1"> No Server found. </td>
                                                                        <td class="td1">  </td> </tr>
