...
$title = "Cohesity Failure/Exception Report from ($($cluster.name.ToUpper())) for $prefix.Toupper()"
$message = '<html>
<head>
    <style>
        body {
...
$message += '<p class="title">{0} Job Failure/Exception Report ({1}) for ({2})</p>' -f $cluster.name.ToUpper(), (Get-Date),$prefix.toupper()
$failureCount = 0
