...
$title = "Daily Incremental Backup failure Report "
...
$jobs = api get "protectionJobs?isDeleted=false&isActive=true"|Where-Object name -like "*sql*"
...
<p style="margin-top: 15px; margin-bottom: 15px;"><span style="font-size:1em;">' + $totalFailedObjects + ' SQL servers backup failed out of ' + $totalObjects + ' total Servers (' + $percentFailed + ' success rate)</span></p>               
</div>
</body>
</html>
'

$fileName = "./strikeReport-SQL.html"
$html | out-file $fileName

#write-host "sending report to $([string]::Join(", ", $sendTo))"
### send email report
foreach($toaddr in $sendTo){
    Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject "Daily Incremental Backup failure Report" -BodyAsHtml $html -Attachments $fileName -WarningAction SilentlyContinue
}