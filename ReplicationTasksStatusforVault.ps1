    ...
    foreach($run in (api get -v2 "data-protect/protection-groups/$($job.id)/runs?startTimeUsecs=$daysBackUsecs&numRuns=$numRuns&includeTenants=true&includeObjectDetails=false").runs){
        ...
        if($run.PSObject.Properties['replicationInfo']){
            foreach($replication in $run.replicationInfo.replicationTargetResults){
                ...
                ###############get last date #############
                $jb = api get protectionJobs | Where-Object {$_.isDeleted -ne $True -and $_.isActive -ne $false -and $_.name -eq $jobname}
                $runningTasks = @()
                  $jobId = $jb.id
                    $thisJobName = $jb.name    
                 "Getting tasks for $thisJobName"
                 $myruns = api get "protectionRuns?jobId=$jobId&numRuns=999&excludeTasks=true" 
                    ...
                    if($runningTasks.Count -gt 0){             
                       $replicationEnd = usecsToDate $runningTasks[0]
                                               }
                #########################################
                      }
