...
function processStats($stats, $name, $environment, $location, $tenant){
    ...
    if ( $totalGrowth -gt 500 )  { $color='FF9292'} elseif ( $totalGrowth -lt 0) {$color = '50f058'} else {$color = '5098f0'}
    $thisHTML = ("<tr><td>{0}</td>
                      <td>{1}</td>
                      <td>{2}</td>
                      <td>{3}</td>
                      <td>{4}</td>
                      <td>{5}</td>
                      <td td bgcolor= '#$($color)'>{6}</td></tr>" -f $name, $tenant, $environment, $location, '', '', $totalGrowth) + $thisHTML
    return $thisHTML
}
