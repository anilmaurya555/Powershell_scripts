...
$outFile = $(Join-Path -Path $PSScriptRoot -ChildPath "Daily_unix_report.csv")
...
<center>AIX and Linux Backup Report from ALL Cohesity clusters<br>
...
$outFilePath = join-path -Path $PSScriptRoot -ChildPath 'dailyunixreport.html'
$html | Out-File -FilePath 'dailyunixreport.html' -Encoding ascii

$attachfile = @("$outfile")
# send email report
#write-host "sending report to $([string]::Join(", ", $sendTo))"
foreach($toaddr in $sendTo){
    Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject "Daily AIX and Linux Backup Report from ALL Cohesity cluster" -BodyAsHtml $html -WarningAction SilentlyContinue -Attachments $attachfile
}
