...
foreach($job in $jobs.protectionGroups | Sort-Object -Property name){ ##1
    if($jobNames.Count -eq 0 -or $job.name -in $jobNames){ #######2  this loop will process if nothing specified for $jobname and $joblist
