...
param (
    ...
    [Parameter()][array]$prefix = 'gpfs', #report jobs with 'prefix' only
...
$outFile = $(Join-Path -Path $PSScriptRoot -ChildPath "Daily_gpfs_report.csv")
"Cluster Name,Object Name,Available Snapshots,Last Start Time,Last End Time,Logical MB,Read MB,Written MB" | Out-File -FilePath $outFile
...
<center>GPFS NON ACL backup Report from ALL Cohesity cluster<br>
...
<tbody><tr><td colspan="8" align="CENTER" valign="TOP" bgcolor="#000080"><font size="+1" color="#FFFFFF">Summary</font></td></tr><tr bgcolor="#FFFFFF">'
...
$clusters = ('hcohesity04','hcohesity05')
...
foreach ($entity in $objectStatus.Keys | Sort-Object){
   ...
   # $html += td $lastRunErrorMsg $color
    $html += '</tr>'
    "{0},{1},{2},{3},{4},{5},{6},{7}" -f $server,$objectName,$numSnapshots,$lastRunStartTime,$endTime,$displaySize,$displayRead,$displayWritten | out-file -FilePath $outfile -Append
...
$outFilePath = join-path -Path $PSScriptRoot -ChildPath 'dailygpfsreport.html'
$html | Out-File -FilePath 'dailygpfsreport.html' -Encoding ascii

$attachfile = @("$outfile")
# send email report
#write-host "sending report to $([string]::Join(", ", $sendTo))"
foreach($toaddr in $sendTo){
    Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject "GPFS NON ACL Backup Report from ALL Cohesity cluster" -BodyAsHtml $html -WarningAction SilentlyContinue -Attachments $attachfile
}
