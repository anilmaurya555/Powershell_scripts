...
1..6 | ForEach {
    ...
    $worksheet.Name = "Hcohesity01 - Usage"
    ...
    ### headings for data rows
        ...
        ### authenticate
        apiauth -vip Hcohesity01 -username $username -domain corpads.local
        ...
        $worksheet.usedRange.rows(1).Font.Bold = $True
                      ...
                      $worksheet = $workbook.Worksheets.Item($_)
                      $worksheet.Name = "Hcohesity03 - Usage"
                      $worksheet.activate|Out-Null
                          ### headings for data rows
                        $row = 1
                        $worksheet.Cells.Item($row,1) = 'Date'
                        ...
                        $worksheet.usedRange.rows(1).Font.Bold = $True
                                          ...
                                          $worksheet = $workbook.Worksheets.Item($_)
                                          $worksheet.Name = "Hcohesity04 - Usage"
                                          $worksheet.activate|Out-Null
                                              ### headings for data rows
                                            $row = 1
                                            $worksheet.Cells.Item($row,1) = 'Date'
                                            ...
                                            ### authenticate
                                            apiauth -vip Hcohesity04 -username $username -domain corpads.local
                                            ...
                                            $worksheet.usedRange.rows(1).Font.Bold = $True
                                                 ...
                                                 $worksheet = $workbook.Worksheets.Item($_)
                                                 $worksheet.Name = "Hcohesity05 - Usage"
                                                 $worksheet.activate|Out-Null
                                                     ### headings for data rows
                                                    $row = 1
                                                    $worksheet.Cells.Item($row,1) = 'Date'
                                                    ...
                                                    ### authenticate
                                                    apiauth -vip Hcohesity05 -username $username -domain corpads.local
                                                    ...
                                                    $worksheet.columns.autofit() | Out-Null
                                                    $worksheet.usedRange.rows(1).Font.Bold = $True
                                                 }   elseif ($_ -eq 5){   ###ibmcos1
                                                                                                    $worksheet = $workbook.Worksheets.Item($_)
                                                                 $worksheet.Name = "Hcohesity01-IBMCOS - Usage"
                                                                 $worksheet.activate|Out-Null
                                                                     ### headings for data rows
                                                                    $row = 1
                                                                    $worksheet.Cells.Item($row,1) = 'Date'
                                                                    ...
                                                                    $vault = $vaults | Where-Object name -eq "HC1Archive1_ICOS"

                                                                    ### collect $days of write throughput stats
                                                                    #$stats = api get statistics/timeSeriesStats?schemaName=kBridgeClusterStats`&entityId=$clusterId`&metricName=kSystemUsageBytes`&startTimeMsecs=$startTimeMsecs`&rollupFunction=average`&rollupIntervalSecs=86400
                                                                    $stats = api get "statistics/timeSeriesStats?endTimeMsecs=1662609600000&entityId=$($vault.id)&metricName=kMorphedUsageBytes&metricUnitType=0&range=day&rollupFunction=average&rollupIntervalSecs=86400&schemaName=kIceboxVaultStats&startTimeMsecs=$startTimeMsecs"
                                                                    ...
                                                                    $worksheet.usedRange.rows(1).Font.Bold = $True
                                                                          ...
                                                                          $worksheet = $workbook.Worksheets.Item($_)
                                                                         $worksheet.Name = "Hcohesity03-IBMCOS - Usage"
                                                                         $worksheet.activate|Out-Null
                                                                             ### headings for data rows
                                                                            $row = 1
                                                                            $worksheet.Cells.Item($row,1) = 'Date'
                                                                            ...
                                                                            $clusterId = $clusterInfo.id
                                                                            $vaults = api get vaults
                                                                            $vault = $vaults | Where-Object name -eq "HC3Archive1_ICOS"
                                                                            ### collect $days of write throughput stats
                                                                            #$stats = api get statistics/timeSeriesStats?schemaName=kBridgeClusterStats`&entityId=$clusterId`&metricName=kSystemUsageBytes`&startTimeMsecs=$startTimeMsecs`&rollupFunction=average`&rollupIntervalSecs=86400
                                                                            $stats = api get "statistics/timeSeriesStats?endTimeMsecs=1662609600000&entityId=$($vault.id)&metricName=kMorphedUsageBytes&metricUnitType=0&range=day&rollupFunction=average&rollupIntervalSecs=86400&schemaName=kIceboxVaultStats&startTimeMsecs=$startTimeMsecs"
...
foreach($toaddr in $sendTo){
   Send-MailMessage -From $sendFrom -To $toaddr -SmtpServer $smtpServer -Port $smtpPort -Subject "Last Six Months Cohesity usage Stats." -BodyAsHtml $html  -WarningAction SilentlyContinue -Attachments $xlsx }
