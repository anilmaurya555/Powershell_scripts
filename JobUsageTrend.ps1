...
param (
    ...
    [Parameter()][ValidateSet("Daily","Weekly","Monthly")][string]$rollup = 'Daily' ############## show by daily, by weekly or by monthly ##############
)
...
$outFile = Join-Path -Path $PSScriptRoot -ChildPath "$($cluster.name)-JobUsageTrend.txt"
