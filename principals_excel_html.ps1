...
$vips = ('chyukrccp01','chyukpccp01','chyididcp01')
...
$parents = @{}
$privileges = ($roles |where {$_.label -eq "Admin"}).privileges
foreach ($pri in $privileges){
  ...
  foreach ($dom in $cluster.value.GetEnumerator()){ ###dom
       $domain = $dom.name
            foreach ($prin in $dom.value.GetEnumerator()){
              ...
              if($admin){
                if ($prin.value.permission -like "*Admin*"){
                "{0} {1} {2} {3} {4} " -f $vip,$type,$prin.name,$prin.value.description,  $prin.value.permission    
                ...
                $html += "
                                                 ...
                                                 <td>$domain</td>
                                                 <td>$($prin.name)</td>
                                                 <td>$($prin.value.description)</td>
                                                 <td>$($prin.value.permission)</td>
                                                 </tr>" }
                        }else{
...
$html += "<td>             <!start of second cell in frist row>
<table >          <! start of right table>
<tr><th>Privileges</th></tr>
<tr><td >$newpriv</td></tr>
</table>         <! end of right table >
</td>            <! end of main cell which was divided between two table>
</tr>            <! end of 1st row>
</table>         <! end of main table>
</body>
</html>"

$outfilepath = "Account_privileges.html"
