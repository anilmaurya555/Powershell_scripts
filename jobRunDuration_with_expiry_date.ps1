...
"=============================================================================================================================================="
"JobName                                           Start Time            Duration in Seconds      Data Read in MB       Epire Time"
"=============================================================================================================================================="
...
foreach($jobName in ($jobNames | Sort-Object -Property name)){
    ...
    if ($lastnumdays){
        ...
        foreach($run in $runs | Where-Object {$_.backupRun.status -eq 'kSuccess'}){
            ...
            #"{0,-50}{1,-20}`t {2,-10}`t MB read `t {3,-10}  `t {4,-10}" -f $jobName, $startTime, $durationSeconds, $dataReadMBytes,$expiretime
            "{0,-50}{1,-20}`t {2,-10} `t `t`t {3,-10} `t`t {4,-10} " -f $jobName, $startTime,$durationSeconds, $dataReadMBytes,$expire
