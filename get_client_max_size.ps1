              ...
              if ( $diff.Days -lt 400 ) {
                   if ( $arr[12] -ne "Clone_DR") {
                        if ($arr[0] -notin $ht.keys){
                                                 ...
                                                 if ($btype -match "DB2") {  $ht[$arr[0]]['Db2size'] = $arr[1] }
                                                 if ($btype -match "RMAN") { $ht[$arr[0]]['RMANsize'] = $arr[1]} 
                                                 if  ($btype -notmatch "RMAN" -and $btype -notmatch "DB2" ){$ht[$arr[0]]['Filesize'] = $arr[1]}
                                                 
                                                 } else {
                                                         
                                                if ($btype -match "DB2" -and $arr[1] -gt $ht[$arr[0]]['Db2size'] ) {$ht[$arr[0]]['Db2size'] = $arr[1]}                                                                      
                                                if ($btype -match "RMAN" -and $arr[1] -gt $ht[$arr[0]]['RMANsize']) {$ht[$arr[0]]['RMANsize'] = $arr[1]  } 
                                                if  ($btype -notmatch "RMAN" -and $btype -notmatch "DB2" -and $arr[1] -gt $ht[$arr[0]]['Filesize']){  $ht[$arr[0]]['Filesize'] = $arr[1]}
...
write-host "======================================================================================"
 ...
 "{0,-25}     {1:n2}          {2:n2}            {3:n2}            {4:n2}" -f $_.name,$total,$($_.value.Db2size/1024/1024),$($_.value.RMANsize/1024/1024),$($_.value.Filesize/1024/1024)}  