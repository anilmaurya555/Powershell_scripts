...
write-host "JobName,JobType,Status,RunDate,RunType,DurationSec,LogicalMB,DataReadMB,DataWrittenMB,RunURL"
#write-host "`t{0}`t{1}`t{2}`t{3}`t{4}`t{5}" -f (usecsToDate $startTime), $status, $runType, $duration, $readMBytes
...
foreach ($job in $jobs){
    ...
    foreach ($run in $runs){
        ...
        if($nowTime - $startTime -le 86400000000){
